...
// A key is valid when it is a purely relative, normalised path: no leading
// slash, no empty segments (`a//b`), no `.`/`..` segments, no backslash or NUL,
// and every component is a plain `Normal` component on this platform (rejects
// Windows prefixes such as `C:` or `\\?\` as well).
pub fn valid_key(key: &str) -> bool {
    ...
    // Belt and braces: whatever the string checks above say, the OS path
    // parser must agree that every component is an ordinary name.
    Path::new(key)
        .components()
        .all(|c| matches!(c, std::path::Component::Normal(_)))
}

// Version ids are single path components generated by `new_version_id`; a
// client-supplied one must never be able to walk out of `versions/<key>/`.
pub fn valid_version_id(vid: &str) -> bool {
    !vid.is_empty()
        && vid.len() <= 128
        && vid
            .bytes()
...
impl Storage {
    ...
    // Single choke point for every operation addressed by (bucket, key).
    // Validates both names and then proves that the joined data/meta/versions
    // paths are still inside the bucket's respective directories, so that no
    // later `Path::join` can escape the storage root.
    pub(crate) fn check_object(&self, bucket: &str, key: &str) -> Result<(), StorageError> {
        if !valid_bucket(bucket) || !valid_key(key) {
            return Err(StorageError::InvalidName);
        }
        let dir = self.bucket_dir(bucket);
        let inside = [
            (self.data_path(bucket, key), dir.join("data")),
            (self.meta_path(bucket, key), dir.join("meta")),
            (self.versions_dir(bucket, key), dir.join("versions")),
        ];
        for (p, base) in &inside {
    ...
    fn check_version(&self, bucket: &str, key: &str, vid: &str) -> Result<(), StorageError> {
        self.check_object(bucket, key)?;
    ...
    pub fn bucket_exists(&self, bucket: &str) -> bool {
        // Invalid names can never be buckets; this also stops a traversal
        // bucket name (e.g. from virtual-hosted addressing) probing the fs.
        valid_bucket(bucket) && self.marker(bucket).exists()
    }
    ...
    pub fn get_object_version(
        ...
        self.check_version(bucket, key, version_id)?;
        if !self.bucket_exists(bucket) {
            return Err(StorageError::NotFound);
        }
    ...
    pub fn is_delete_marker(&self, bucket: &str, key: &str, version_id: &str) -> bool {
        if self.check_version(bucket, key, version_id).is_err() {
            return false;
        }
...
mod tests {
    ...
    #[test]
    fn valid_key_rejects_traversal_shapes() {
        // Absolute keys would replace the base path in `Path::join`.
        assert!(!valid_key("/etc/passwd"));
        assert!(!valid_key("/"));
        assert!(!valid_key("//"));
        // Parent references anywhere.
        assert!(!valid_key(".."));
        assert!(!valid_key("../x"));
        assert!(!valid_key("a/../b"));
        assert!(!valid_key("a/.."));
        // Empty segments and trailing slash.
        assert!(!valid_key("a//b"));
        assert!(!valid_key("a/"));
        // Legitimate S3 keys keep working.
        assert!(valid_key("dir/sub/file.txt"));
        assert!(valid_key("with space.txt"));
        assert!(valid_key("a+b%20c"));
        assert!(valid_key("ünïcødé/файл.txt"));
        assert!(valid_key("a..b/..c/d.."));
        assert!(valid_key(".hidden"));
    }

    #[test]
    fn valid_version_id_rules() {
        assert!(valid_version_id(&new_version_id()));
        assert!(valid_version_id("abc-123_x.y"));
        assert!(!valid_version_id(""));
        assert!(!valid_version_id(".."));
        assert!(!valid_version_id("../x"));
        assert!(!valid_version_id("a/b"));
        assert!(!valid_version_id("a\\b"));
        assert!(!valid_version_id("a\0b"));
    }

    // A sentinel file outside the storage root must be neither readable nor
    // deletable through any key-addressed storage entry point.
    #[test]
    fn traversal_keys_cannot_reach_outside_root() {
        let (s, g) = fresh("traversal");
        s.create_bucket("buck").unwrap();
        s.set_versioning_status("buck", VersioningStatus::Enabled)
            .unwrap();

        // Sentinel next to the root (sibling directory, so `..` from data/ can reach it).
        let sentinel_dir = tmp_root("traversal_sentinel");
        fs::create_dir_all(&sentinel_dir).unwrap();
        let _sentinel_guard = ScopedRoot(sentinel_dir.clone());
        let sentinel = sentinel_dir.join("secret.txt");
        fs::write(&sentinel, b"top secret").unwrap();
        let abs_key = sentinel.to_str().unwrap().to_string();
        let sentinel_name = sentinel_dir.file_name().unwrap().to_str().unwrap();
        // data/ is at <root>/buckets/buck/data, so five `..` reach temp_dir().
        let rel_key = format!("../../../../../{}/secret.txt", sentinel_name);
        let _ = &g;

        for key in [abs_key.as_str(), rel_key.as_str(), "a/../b", "a//b"] {
            assert!(
                matches!(s.get_object("buck", key), Err(StorageError::InvalidName)),
                "get_object accepted {:?}",
                key
            );
            assert!(
                matches!(
                    s.put_object_writer("buck", key),
                    Err(StorageError::InvalidName)
                ),
                "put_object_writer accepted {:?}",
                key
            );
            assert!(
                matches!(s.delete_object("buck", key), Err(StorageError::InvalidName)),
                "delete_object accepted {:?}",
                key
            );
            assert!(
                matches!(
                    s.get_object_version("buck", key, "v1"),
                    Err(StorageError::InvalidName)
                ),
                "get_object_version accepted {:?}",
                key
            );
            assert!(
                matches!(
                    s.delete_object_version("buck", key, "v1"),
                    Err(StorageError::InvalidName)
                ),
                "delete_object_version accepted {:?}",
                key
            );
            assert!(!s.is_delete_marker("buck", key, "v1"));
        }

        // Traversal via the version id component.
        assert!(matches!(
            s.get_object_version("buck", "k", "../../../x"),
            Err(StorageError::InvalidName)
        ));
        assert!(matches!(
            s.delete_object_version("buck", "k", "../../../x"),
            Err(StorageError::InvalidName)
        ));

        // Traversal via the bucket name.
        assert!(!s.bucket_exists("../buckets/buck"));
        assert!(matches!(
            s.get_object("../buckets/buck", "k"),
            Err(StorageError::InvalidName)
        ));

        assert_eq!(fs::read(&sentinel).unwrap(), b"top secret");
    }
