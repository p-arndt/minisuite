// Stamp / bump the version across minimail's manifests.
//
...
//
// Patches Cargo.toml (the [package] version) and Cargo.lock (minimail's own
// package entry) via targeted regex replaces, so existing formatting and
// comments are left untouched. minimail's default build is dependency-free
// (rustls only under the optional `tls` feature), so the name-anchored regex
// still matches its own lockfile entry and a version bump needs no network
// refresh.
//
...
const TARGETS = [
  ...
  // minimail's own entry in the lockfile.
  { file: "Cargo.lock", re: /(name = "minimail"\r?\nversion = )"[^"]*"/ },
];
